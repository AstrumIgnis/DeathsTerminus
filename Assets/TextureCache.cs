...
namespace DeathsTerminus.Assets
{
    public static class TextureCache
    {
        public static Ref<Texture2D> EclipseTexture { get; internal set; }
        public static Ref<Texture2D> BlueSunTexture { get; internal set; }
        public static Ref<Texture2D> RainbowSunTexture { get; internal set; }

        internal static void Async_CreateSunTextures(object callContext)
        {
            try
            {
                ...
                EclipseTexture = new Ref<Texture2D>(new Texture2D(Main.spriteBatch.GraphicsDevice, eclipseFrameSize * textureFramesX, eclipseFrameSize * textureFramesY, false, SurfaceFormat.Color));
                List<Color> list = new List<Color>();
                for (int k = 0; k < textureFramesY; k++)
                {
                    for (int j = 0; j < EclipseTexture.Value.Height / textureFramesY; j++)
                    {
                        for (int l = 0; l < textureFramesX; l++)
                        {
                            for (int i = 0; i < EclipseTexture.Value.Width / textureFramesX; i++)
                            {
                                float x = (2 * i / (float)(EclipseTexture.Value.Width / textureFramesX - 1) - 1);
                                float y = (2 * j / (float)(EclipseTexture.Value.Height / textureFramesY - 1) - 1);
                ...
                if (DeathsTerminus.ReleaseAsync)
                    return;
                EclipseTexture.Value.SetData(list.ToArray());
                logger.Info("Finished the (" + nameof(EclipseTexture) + ") texture.");

                if (DeathsTerminus.ReleaseAsync)
                    return;
                BlueSunTexture = new Ref<Texture2D>(new Texture2D(Main.spriteBatch.GraphicsDevice, eclipseFrameSize * textureFramesX, eclipseFrameSize * textureFramesY, false, SurfaceFormat.Color));
                list = new List<Color>();
                for (int k = 0; k < textureFramesY; k++)
                {
                    for (int j = 0; j < EclipseTexture.Value.Height / textureFramesY; j++)
                ...
                if (DeathsTerminus.ReleaseAsync)
                    return;
                BlueSunTexture.Value.SetData(list.ToArray());

                logger.Info("Finished the (" + nameof(BlueSunTexture) + ") texture.");

                if (DeathsTerminus.ReleaseAsync)
                    return;
                RainbowSunTexture = new Ref<Texture2D>(new Texture2D(Main.spriteBatch.GraphicsDevice, eclipseFrameSize * textureFramesX, eclipseFrameSize * textureFramesY, false, SurfaceFormat.Color));
                list = new List<Color>();
                for (int k = 0; k < textureFramesY; k++)
                {
                    for (int j = 0; j < EclipseTexture.Value.Height / textureFramesY; j++)
                ...
                if (DeathsTerminus.ReleaseAsync)
                    return;
                RainbowSunTexture.Value.SetData(list.ToArray());

                logger.Info("Finished the (" + nameof(RainbowSunTexture) + ") texture.");
            }
            catch
            {
            }
